...
shared_examples "a timeline" do
  describe "ticks" do
    ...
    let(:unlabeled_ticks) { subject.ticks.select(&:labeled?) }

    it "have at least 10 labeled ticks" do
      expect(subject.ticks.count(&:labeled?)).to(be >= 10)
    end

    it "has labeled ticks every 4 other tick" do
      expect(labeled_ticks).to(all(satisfy(&:labeled?)))
    end
...
describe Timeline do
  describe "#initialize" do
    context "when the duration is less than 45 seconds" do
      subject(:timeline) { build(:timeline, :less_than_forty_five_seconds) }

      it "is a timeline" do
        expect(timeline.ticks.size).to(be >= 11)
      end
    end

    context "when the duration is ten minutes" do
      subject(:timeline) { build(:timeline, :ten_minutes) }

      it_behaves_like "a timeline"

      it "has the last item be 10 minutes" do
        expect(timeline.ticks.last.value).to(eq(10.minutes.in_seconds))
      end

      it "has the penultimate item be .2 seconds less" do
        expect(timeline.ticks[timeline.ticks.size - 2].value.seconds.in_minutes).to(eq(9.8))
      end
    end
    ...
    context "when the timeline starts on non-zero value" do
      ...
      it "starting value is equal to or less than the starting value" do
